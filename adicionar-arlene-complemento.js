// Script para adicionar dados complementares da agente "Arlene" no PostgreSQL
...
//   node adicionar-arlene-complemento.js
...
const dadosArleneComplemento = [
  ...
  ['Carteira Ativa', 2026, 'Janeiro', '303.120'],
  ['Clientes Ativos', 2026, 'Janeiro', '159'],
  ['Inadimplência', 2026, 'Janeiro', '5,06'],
  ['Liberações', 2026, 'Janeiro', '109.200']
];
...
async function main() {
  ...
  try {
    console.log(`Inserindo registros complementares para a agente ${pessoa} (ignorando duplicados)...`);
    ...
    try {
      ...
      for (const [categoria, ano, mes, valorBruto] of dadosArleneComplemento) {
      ...
      console.error('Erro ao inserir registros para Arlene:', e);
    } finally {
      client.release();
    }
  } catch (e) {
    console.error('Erro geral no script adicionar-arlene-complemento:', e);
    process.exit(1);
  } finally {
    await pool.end();
  }
