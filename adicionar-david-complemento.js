// Script para adicionar dados complementares do agente "David" no PostgreSQL
...
//   node adicionar-david-complemento.js
...
const dadosDavidComplemento = [
  ['Carteira Ativa', 2025, 'Dezembro', '141.191'],
  ['Clientes Ativos', 2025, 'Dezembro', '113'],
  ['Inadimplência', 2025, 'Dezembro', '20,65'],

  ['Carteira Ativa', 2026, 'Janeiro', '159.947'],
  ['Clientes Ativos', 2026, 'Janeiro', '112'],
  ['Inadimplência', 2025, 'Janeiro', '19,77'],
  ['Liberações', 2026, 'Janeiro', '57.900']
];
...
async function main() {
  ...
  try {
    ...
    try {
      ...
      for (const [categoria, ano, mes, valorBruto] of dadosDavidComplemento) {
      ...
      console.error('Erro ao inserir registros para David:', e);
    } finally {
      client.release();
    }
  } catch (e) {
    console.error('Erro geral no script adicionar-david-complemento:', e);
    process.exit(1);
  } finally {
    await pool.end();
  }
